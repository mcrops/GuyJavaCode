...
public class StoryUploaderActivity extends Activity implements StoryUploaderListener {
    private final static String t = "UploaderActivity";
    ...
    private StoryUploaderTask mInstanceUploaderTask;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null && savedInstanceState.containsKey(TO_SEND)) {
            ...
            selectedInstanceIDs = intent.getLongArrayExtra("instances");
        }
        ...
        mInstanceUploaderTask = (StoryUploaderTask) getLastNonConfigurationInstance();
        if (mInstanceUploaderTask == null) {
            // setup dialog and upload task
            showDialog(PROGRESS_DIALOG);
            mInstanceUploaderTask = new StoryUploaderTask();
            mInstanceUploaderTask.ctx = getApplicationContext();
            mInstanceUploaderTask.preferences = preferences;

            // register this activity with the new uploader task
            mInstanceUploaderTask.setUploaderListener(StoryUploaderActivity.this);
    ...
    public void uploadingComplete(HashMap<String, String> result) {
        ...
        while (it.hasNext()) {
            String id = it.next();
            selection.append(StoryColumns._ID + "=?");
            selectionArgs[i++] = id;
            if (i != keys.size()) {
                selection.append(" or ");
            }
        ...
        StringBuilder message = new StringBuilder();
        {
        	Cursor results = null;
        	try {
                results = getContentResolver().query(StoryColumns.CONTENT_URI,
                		null, selection.toString(), selectionArgs, null);
                if (results.getCount() > 0) {
                    results.moveToPosition(-1);
                    while (results.moveToNext()) {
                        String name =
                            results.getString(results.getColumnIndex(StoryColumns.TITLE));
                        String id = results.getString(results.getColumnIndex(StoryColumns._ID));
                        message.append(name + " - " + result.get(id) + "\n\n");
                    }
                } else {
                    message.append(getString(R.string.no_stories_uploaded));
                }
        	} finally {
        		if ( results != null ) {
        			results.close();
        ...
        switch (id) {
                ...
                DialogInterface.OnClickListener loadingButtonListener =
                    new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                        	MandeUtility.Log(false, "onCreateDialog.PROGRESS_DIALOG:cancel");
                            dialog.dismiss();
            ...
            case AUTH_DIALOG:
                ...
                b.setPositiveButton(R.string.ok, new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                    	MandeUtility.Log(false, "onCreateDialog.AUTH_DIALOG:OK");
                        EditText username = (EditText) dialogView.findViewById(R.id.username_edit);
                        EditText password = (EditText) dialogView.findViewById(R.id.password_edit);
                ...
                b.setNegativeButton(R.string.cancel, new DialogInterface.OnClickListener() {
                    ...
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                    	MandeUtility.Log(false, "onCreateDialog.AUTH_DIALOG:cancel");
                        finish();
                    }
                });
